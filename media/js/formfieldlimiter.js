...
uncheckedkeycodes: /(8)|(13)|(16)|(17)|(18)|(32)/, //keycodes that are not checked, even when limit has been reached. See http://www.javascriptkit.com/jsref/eventkeyboardmouse.shtml for avail keycodes
...
limitinput:function(e, config){
	...
	if (!this.uncheckedkeycodes.test(keyunicode)){
        if(navigator.userAgent.indexOf("Firefox")!=-1){
            var split = thefield.value.split("\n")
            a = split.length -1
		...
		if (thefield.value.length + a >=config.maxlength){
			if (e.preventDefault)
				e.preventDefault()
			return false
		}
...
showlimit:function(config){
    ...
    if(navigator.userAgent.indexOf("Firefox")!=-1)
    {
         ...
         charsleft=config.maxlength-thefield.value.length
    }
	...
	if (charsleft<0) //if user has exceeded input limit (possible if cut and paste text into field)
        ...
        if(navigator.userAgent.indexOf("Firefox")!=-1){
            ...
            thefield.value=thefield.value.substring(0, config.maxlength - a) //trim input
        }
        else{
