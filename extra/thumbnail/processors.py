...
from extra.thumbnail import utils
import re
...
def autocrop(im, requested_size, opts):
    if 'autocrop' in opts and not 'nocrop' in opts:
        bw = im.convert("1")
        ...
        if bbox:
            im = im.crop(bbox)
    return im
autocrop.valid_options = ('autocrop','nocrop')
...
def scale_and_crop(im, requested_size, opts):
    ...
    if 'nocrop' not in opts and ('crop' in opts or 'max' in opts):
        r = max(xr / x, yr / y)
    else:
        r = min(xr / x, yr / y)
    ...
    if crop and not 'nocrop' in opts:
    ...
    return im
scale_and_crop.valid_options = ('nocrop','crop', 'upscale', 'max')
