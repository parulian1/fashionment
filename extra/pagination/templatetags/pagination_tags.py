...
def paginate(context, url='', window=DEFAULT_WINDOW):
    """
    ...
    try:
        ...
        if len(first.intersection(current)) == 0:
            ...
            second_list.sort()
            page_numbers.extend(first_list)
            diff = second_list[0] - first_list[-1]
            # If there is a gap of two, between the last page of the first
            # set and the first page of the current set, then we're missing a
            # page.
            if diff == 2:
                page_numbers.append(second_list[0] - 1)
            ...
            page_numbers.extend(second_list)
        else:
            unioned = list(first.union(current))
            unioned.sort()
            page_numbers.extend(unioned)
        # If there's no overlap between the current set of pages and the last
        # set of pages, then there's a possible need for elusion.
        if len(current.intersection(last)) == 0:
            second_list = list(last)
            second_list.sort()
            diff = second_list[0] - page_numbers[-1]
            ...
            differenced.sort()
            page_numbers.extend(differenced)
        to_return = {
